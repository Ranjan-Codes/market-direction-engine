...
export function GaugeDial({ direction, intensity }: { direction: string; intensity: number }) {
  ...
  return (
    <svg width={120} height={66}>
      <path d={`M ${cx - r} ${cy} A ${r} ${r} 0 0 1 ${cx + r} ${cy}`} fill="none" stroke="#3f3f46" strokeWidth={9} />
      <path
        d={`M ${cx - r} ${cy} A ${r} ${r} 0 0 1 ${cx + r * Math.cos(angle)} ${cy - r * Math.sin(angle)}`}
        fill="none" stroke={color} strokeWidth={9} strokeLinecap="round"
      />
      <line x1={cx} y1={cy} x2={nx} y2={ny} stroke="#e4e4e7" strokeWidth={2} />
...
export function Panel({
  ...
  return (
    <section className="border border-zinc-800 rounded bg-zinc-950">
      <header className="flex items-center justify-between px-3 py-1.5 border-b border-zinc-800">
